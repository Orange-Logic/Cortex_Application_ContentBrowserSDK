...
const ht = (n) => new tt(typeof n == "string" ? n : n + "", void 0, I), wt = (n, ...t) => {
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const { is: lt, defineProperty: ct, getOwnPropertyDescriptor: dt, getOwnPropertyNames: pt, getOwnPropertySymbols: $t, getPrototypeOf: ut } = Object, f = globalThis, q = f.trustedTypes, _t = q ? q.emptyScript : "", M = f.reactiveElementPolyfillSupport, b = (n, t) => n, B = { toAttribute(n, t) {
  switch (t) {
    case Boolean:
      n = n ? _t : null;
      break;
  ...
  return e;
} }, et = (n, t) => !lt(n, t), J = { attribute: !0, type: String, converter: B, reflect: !1, hasChanged: et };
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const C = globalThis, N = C.trustedTypes, K = N ? N.createPolicy("lit-html", { createHTML: (n) => n }) : void 0, st = "$lit$", _ = `lit$${Math.random().toFixed(9).slice(2)}$`, it = "?" + _, ft = `<${it}>`, g = document, w = () => g.createComment(""), P = (n) => n === null || typeof n != "object" && typeof n != "function", j = Array.isArray, At = (n) => j(n) || typeof (n == null ? void 0 : n[Symbol.iterator]) == "function", k = `[ 	
\f\r]`, S = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, Z = /-->/g, F = />/g, A = RegExp(`>|${k}(?:([^\\s"'>=/]+)(${k}*=${k}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), G = /'/g, Q = /"/g, nt = /^(?:script|style|textarea|title)$/i, rt = (n) => (t, ...e) => ({ _$litType$: n, strings: t, values: e }), Ut = rt(1), xt = rt(2), v = Symbol.for("lit-noChange"), d = Symbol.for("lit-nothing"), X = /* @__PURE__ */ new WeakMap(), m = g.createTreeWalker(g, 129);
function ot(n, t) {
...
const mt = (n, t) => {
  ...
  for (let l = 0; l < e; l++) {
    const h = n[l];
    let c, p, a = -1, $ = 0;
    for (; $ < h.length && (r.lastIndex = $, p = r.exec(h), p !== null); ) $ = r.lastIndex, r === S ? p[1] === "!--" ? r = Z : p[1] !== void 0 ? r = F : p[2] !== void 0 ? (nt.test(p[2]) && (i = RegExp("</" + p[2], "g")), r = A) : p[3] !== void 0 && (r = A) : r === A ? p[0] === ">" ? (r = i ?? S, a = -1) : p[1] === void 0 ? a = -2 : (a = r.lastIndex - p[2].length, c = p[1], r = p[3] === void 0 ? A : p[3] === '"' ? Q : G) : r === Q || r === G ? r = A : r === Z || r === F ? r = S : (r = A, i = void 0);
    const u = r === A && n[l + 1].startsWith("/>") ? " " : "";
    o += r === S ? h + ft : a >= 0 ? (s.push(c), h.slice(0, a) + st + h.slice(a) + _ + u) : h + _ + (a === -2 ? l : u);
  }
      ...
      if (i.nodeType === 1) {
        ...
        if (nt.test(i.tagName)) {
          ...
          if ($ > 0) {
            ...
            for (let u = 0; u < $; u++) i.append(a[u], w()), m.nextNode(), h.push({ type: 2, index: ++o });
            i.append(a[$], w());
          }
        }
