...
import { ControlOption, SortOrder } from '@/types/dam-view';
...
import styles from './dam-view-control-sort.styles';
 ...
 *
 * @event cx-dam-view-control-sort-change - Emitted when the sort field or sort order is changed.
 */
@customElement('cx-dam-view-control-sort')
export default class CxDamViewControlSort extends CortexElement {
  ...
  private handleSortSelect(event: CxSelectEvent<CxMenuItem>) {
    ...
    if (type === OptionType.SORT_DIRECTION) {
      this.emit('cx-dam-view-control-sort-order-change', {
        detail: {
          sortDirection: value,
