import './dam-view-control-sort';

import { elementUpdated, expect, fixture, html, oneEvent } from '@open-wc/testing';

import type CxMenuItem from '@orangelogic/design-system/components/menu-item';
import { OptionType, type SortOrder } from '@/types/dam-view';

import type CxDamViewControlSort from './dam-view-control-sort';
...
function getDropdown(el: CxDamViewControlSort) {
  return el.shadowRoot!.querySelector('cx-dropdown')!;
}

function getMenuItems(el: CxDamViewControlSort) {
  return [...getDropdown(el).querySelectorAll('cx-menu-item')] as CxMenuItem[];
}

function findMenuItem(el: CxDamViewControlSort, match: (item: CxMenuItem) => boolean) {
...
async function dispatchSelectOnDropdown(el: CxDamViewControlSort, item: CxMenuItem) {
...
describe('dam-view-control-sort', () => {
  let el: CxDamViewControlSort;

  beforeEach(async () => {
    el = await fixture(html`<cx-dam-view-control-sort></cx-dam-view-control-sort>`);
  });
  ...
  describe('with sort orders and current sort selection', () => {
    ...
    it('emits cx-dam-view-control-sort-order-change when sort direction is selected', async () => {
      const descending = findMenuItem(
        el,
        (item) =>
          item.dataset.type === 'sort-direction' && item.value === 'descending',
      );
      const evtPromise = oneEvent(el, 'cx-dam-view-control-sort-order-change');
    ...
    it('emits cx-dam-view-control-sort-order-change when sort order group is selected', async () => {
  ...
  describe('handleSortSelect guard', () => {
    ...
    it('does not emit when selection detail is incomplete', async () => {
      let fired = false;
      el.addEventListener('cx-dam-view-control-sort-order-change', () => {
        fired = true;
      });
