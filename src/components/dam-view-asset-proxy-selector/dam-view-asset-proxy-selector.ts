...
import { Parameter } from '@/types/dam-view';
...
import CxDamViewAssetTrackingParameters from '../dam-view-asset-tracking-parameters/dam-view-asset-tracking-parameters';

import styles from './dam-view-asset-proxy-selector.styles';

export const USE_REPRESENTATIVE_VALUE = 'use-representative';
export const CUSTOM_FORMAT_VALUE = 'custom';

@customElement('cx-dam-view-asset-proxy-selector')
export default class CxDamViewAssetProxySelector extends CortexElement {
      ...
      ${when(supportedProxies || this.canUseATS,
        () => html`
          ${when(supportedProxies,
            () => html`
              <cx-menu class="dam-view-asset-proxy-selector__menu">
                ${repeat(this.items,
                  (item) => item.id,
                  (item) => html`
                    <cx-menu-item
                      value=${item.id}
                      class=${classMap({
                        'dam-view-asset-proxy-selector__menu-item': true,
                        'dam-view-asset-proxy-selector__menu-item--disable-hover': !this.disabledSelected && item.id === this.selected,
                      })}
                    >
                      ${when(item.image,
                        () => html`
                          <div slot="prefix" class="dam-view-asset-proxy-selector__menu-item__thumbnail">
                      ...
                      <div>
                        <cx-typography
                          variant="body3"
                          class=${classMap({
                            'dam-view-asset-proxy-selector__menu-item__name': true,
                            'dam-view-asset-proxy-selector__menu-item__name--selected': !this.disabledSelected && item.id === this.selected,
                          })}
                        >
                          ${item.name}
                        </cx-typography>
                        <cx-typography variant="body3" class="dam-view-asset-proxy-selector__menu-item__details">
                          ...
                          ${when(item.extension,
                            () => html`
                              <div class="dam-view-asset-proxy-selector__menu-item__extension-dot"></div>
                          ...
                          ${when(item.cdnName,
                            () => html`
                              <div class="dam-view-asset-proxy-selector__menu-item__extension-dot"></div>
                              <span class="dam-view-asset-proxy-selector__menu-item__cdn-name">${item.cdnName}</span>
                            `,
                            () => nothing,
                          )}
                        </cx-typography>
                  ...
                  () => html`
                    ...
                    >
                      <cx-typography
                        ...
                        class=${classMap({
                          'dam-view-asset-proxy-selector__menu-item__name': true,
                          'dam-view-asset-proxy-selector__menu-item__name--selected': !this.disabledSelected && this.selected === USE_REPRESENTATIVE_VALUE,
                        })}
                      >
                ...
                >
                  ...
                  <div>
                    <cx-typography
                      ...
                      class=${classMap({
                        'dam-view-asset-proxy-selector__menu-item__name': true,
                        'dam-view-asset-proxy-selector__menu-item__name--selected': !this.disabledSelected && this.selected === CUSTOM_FORMAT_VALUE,
                      })}
                    >
                    ...
                    ${when(this.selected === CUSTOM_FORMAT_VALUE,
                      () => html`
                        <cx-typography
                          variant="body3"
                          class="dam-view-asset-proxy-selector__menu-item__details"
                        >
          ...
          ${when(this.canUseTracking,
            () => html`
              <cx-menu class="dam-view-asset-proxy-selector__menu">
                <cx-menu-item value="tracking" class="dam-view-asset-proxy-selector__menu-item">
                  <cx-typography variant="body3" class="proxy__name">
                    ${this.localize.term('trackingParameters')}
                  </cx-typography>
                  <cx-switch
                    class="dam-view-asset-proxy-selector__menu-item__switch"
        ...
        () => html`
          <cx-space
            class="dam-view-asset-proxy-selector__menu-item__warning"
            align-items="center"
            spacing="large"
            wrap="nowrap"
          >
            <cx-icon name="warning"></cx-icon>
            <cx-typography variant="body3" class="dam-view-asset-proxy-selector__menu-item__name">
