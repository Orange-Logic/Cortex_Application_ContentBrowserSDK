...
} from './dam-view-asset-proxy-selector';
...
describe('dam-view-asset-proxy-selector', () => {
  ...
  describe('when no proxies and ATS is off', () => {
    let el: CxDamViewAssetProxySelector;

    beforeEach(async () => {
      el = await fixture(html`<cx-dam-view-asset-proxy-selector></cx-dam-view-asset-proxy-selector>`);
    });
  ...
  describe('with proxy items', () => {
    ...
    it('renders a menu with one item per proxy', () => {
      const items = el.shadowRoot!.querySelectorAll(
        'cx-menu.dam-view-asset-proxy-selector__menu cx-menu-item',
      );
    ...
    it('adds disable-hover on the selected row when disabledSelected is false', async () => {
      ...
      expect(menuItem?.classList.contains('dam-view-asset-proxy-selector__menu-item--disable-hover')).to.be
        .true;
    });
    ...
    it('drops disable-hover and selected name style when disabledSelected is true', async () => {
      ...
      expect(menuItem?.classList.contains('dam-view-asset-proxy-selector__menu-item--disable-hover')).to.be
        .false;
      expect(
        el.shadowRoot!.querySelector('.dam-view-asset-proxy-selector__menu-item__name--selected'),
      ).to.be.null;
    });
  });
  ...
  describe('items plus ATS', () => {
    it('renders two menus when both items and canUseATS are set', async () => {
      const el = await fixture<CxDamViewAssetProxySelector>(
        html`<cx-dam-view-asset-proxy-selector></cx-dam-view-asset-proxy-selector>`,
      );
      el.items = [sampleItem];
      el.canUseATS = true;
      await elementUpdated(el);
      expect(el.shadowRoot!.querySelectorAll('cx-menu.dam-view-asset-proxy-selector__menu').length).to
        .equal(2);
    });
  });
  ...
  describe('tracking parameters', () => {
    ...
    it('renders cx-dam-view-asset-tracking-parameters when tracking is enabled', () => {
      expect(el.shadowRoot!.querySelector('cx-dam-view-asset-tracking-parameters')).to.exist;
    });
    ...
    it('passes trackingParameters to the editor as values', () => {
      ...
      ) as CxDamViewAssetTrackingParameters;
    ...
    it('does not render the editor when enabledTracking is false', async () => {
      ...
      await elementUpdated(el);
      expect(el.shadowRoot!.querySelector('cx-dam-view-asset-tracking-parameters')).to.be.null;
    });
    ...
    it('calls preventDefault on tracking switch click so the menu item is not activated as well', async () => {
      ...
      const switchEl = el.shadowRoot!.querySelector(
        'cx-switch.dam-view-asset-proxy-selector__menu-item__switch',
      );
