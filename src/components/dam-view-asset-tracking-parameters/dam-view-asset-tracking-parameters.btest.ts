import './dam-view-asset-tracking-parameters';
...
import type CxDamViewAssetTrackingParameters from './dam-view-asset-tracking-parameters';

function getRootSpace(el: CxDamViewAssetTrackingParameters) {
  return el.shadowRoot!.querySelector('cx-space.dialog__tracking');
}

function getInput(
  el: CxDamViewAssetTrackingParameters,
  field: 'key' | 'value',
  index: number,
): CxInput {
...
describe('dam-view-asset-tracking-parameters', () => {
  let el: CxDamViewAssetTrackingParameters;

  beforeEach(async () => {
    el = await fixture(
      html`<cx-dam-view-asset-tracking-parameters></cx-dam-view-asset-tracking-parameters>`,
    );
  });
  ...
  describe('with values', () => {
    ...
    it('renders a row with key/value inputs and remove control', () => {
      expect(el.shadowRoot!.querySelectorAll('.dam-view-asset-tracking-parameters__row').length).to
        .equal(1);
    ...
    it('emits cx-dam-view-asset-tracking-parameters-change when key input commits', async () => {
      const input = getInput(el, 'key', 0);
      input.value = 'utm_campaign';
      await elementUpdated(input);
      const evtPromise = oneEvent(el, 'cx-dam-view-asset-tracking-parameters-change');
    ...
    it('emits cx-dam-view-asset-tracking-parameters-change when value input commits', async () => {
    ...
    it('does not emit when remove is clicked without data-tracking-index', async () => {
      let count = 0;
      el.addEventListener('cx-dam-view-asset-tracking-parameters-change', () => {
        count += 1;
      });
