 ...
 * @summary CxDamView
 */
@customElement('cx-dam-view-control-filter-facet')
export default class CxDamViewControlFilterFacet extends CortexElement {
  ...
  render() {
    return html`
      <cx-details open class="dam-view-control-filter-facet">
        <cx-space
          slot="summary"
          align-items="center"
          spacing="x-small"
        ...
        <cx-space direction="vertical">
          ...
          >
            ${repeat(Object.entries(this.mappedSubtypes)
              .slice(0, this.page * CxDamViewControlFilterFacet.itemsPerPage),
              ([key]) => key,
              ([key, value]) => {
            ...
            ${when(
              Object.keys(this.mappedSubtypes).length > this.page * CxDamViewControlFilterFacet.itemsPerPage,
              () => html`
                <cx-space>
