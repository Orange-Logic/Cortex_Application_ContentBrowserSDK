...
describe('content-browser-format-dialog', () => {
  ...
  it('cancel without prior confirm restores the proxy selected before entering custom format', async () => {
    el = await fixture<CxContentBrowserFormatDialog>(
      html`<cx-content-browser-format-dialog
        can-use-proxies
        can-custom-format
    ...
    const proxyBeforeCustomFormat = el.selectedProxy;
    ...
    expect(el.selectedProxy).to.equal(proxyBeforeCustomFormat);
  });

  it('cancel without prior confirm and no proxy selected before custom format resets selectedProxy to empty', async () => {
    el = await fixture<CxContentBrowserFormatDialog>(
      html`<cx-content-browser-format-dialog
        can-use-proxies
        can-custom-format
  ...
  describe('mapFormatConfirmPayload', () => {
    ...
    it('uses asset extension and dimensions in sourceProxyMetadata for TRX proxy', async () => {
      ...
      expect(p.proxyPreference).to.equal('TRX');
