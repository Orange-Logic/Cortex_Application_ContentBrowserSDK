...
/**
 * Same as cx-asset-link-format's default cropModes, minus the "Azure AI" auto-cropping option —
 * that suggestion mode is not supported in the content-builder context.
 */
const CROP_MODES = [
...
/**
 * Same as cx-asset-link-format's default focusModes, minus the "Optical", "Google AI", and
 * "Smart Swatch" auto-focus suggestion modes — those are not supported in the content-builder
 * context.
 */
const FOCUS_MODES = [
  { label: 'Manual focus', value: 'manual' },
...
export default class CxContentBrowserFormatDialog extends CortexElement {
  ...
  /**
   * The proxy id that was active right before switching into Custom Format mode.
   * `selectedProxy` gets overwritten to `CUSTOM_FORMAT_VALUE` while the custom format panel is
   * open, so this is what the embedded `cx-asset-link-format` panel's `selected-proxy` attribute
   * is bound to — otherwise it always resolves to `undefined` and the panel silently defaults
   * to `proxies[0]` instead of the proxy the user had actually selected (e.g. TRX).
   */
  @state()
  private lastProxyBeforeCustomFormat: string = '';
  ...
  private handleCancelCustomFormat() {
    ...
    /**
     * If no transformations have been confirmed, restore whichever proxy was active before
     * entering Custom Format mode instead of leaving selectedProxy on CUSTOM_FORMAT_VALUE.
     */
    this.selectedProxy = (!this.confirmedFormat && this.confirmedTransformations.length === 0)
      ? this.lastProxyBeforeCustomFormat
      : this.selectedProxy;
  }
  ...
  private handleDoneCustomFormat() {
    ...
    /**
     * Pick up whatever proxy the user applied inside the panel's own Format selector, so the
     * confirm payload's source proxy reflects that choice instead of staying on whichever proxy
     * was active before Custom Format mode was entered.
     */
    if (this.assetLinkFormat.selectedProxy) {
      this.lastProxyBeforeCustomFormat = this.assetLinkFormat.selectedProxy;
    }
  ...
  private handleProxyConfirm() {
    ...
    if (this.selectedProxy === CUSTOM_FORMAT_VALUE) {
      /**
       * this.selectedProxy is CUSTOM_FORMAT_VALUE while the custom format panel is open, which
       * never matches a real proxy id — the source proxy for a custom transform is whichever
       * proxy was active before entering custom format mode.
       */
      const sourceProxy = this.filteredProxies?.find((proxy) => {
        return proxy.id === this.lastProxyBeforeCustomFormat;
      });
      const payload = this.mapFormatConfirmPayload(this.asset, sourceProxy);
  ...
  private handleProxySelect(event: CxSelectEvent<CxMenuItem>) {
    ...
    if (value === CUSTOM_FORMAT_VALUE) {
      this.lastProxyBeforeCustomFormat = this.selectedProxy;
        ...
        () => {
          ...
          return html`
            <cx-asset-link-format
              for-cropper="cropper"
              hide-header
              selected-proxy=${ifDefined(this.lastProxyBeforeCustomFormat || undefined)}
    ...
    if (!this.canUseProxies) {
      content = html`
        ...
        >
          ...
          <span
            style=${styleMap({
              textTransform: this.ctaTextTransform,
            })}
          >
