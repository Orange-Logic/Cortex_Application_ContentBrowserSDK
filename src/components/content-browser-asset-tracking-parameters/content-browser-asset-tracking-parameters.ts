...
import type { Parameter } from '@/types/content-browser';
...
import styles from './content-browser-asset-tracking-parameters.styles';
 ...
 *
 * @event cx-content-browser-asset-tracking-parameters-change - Fired when parameters are added, removed, or edited. `detail.values` is the new list.
 */
@customElement('cx-content-browser-asset-tracking-parameters')
export default class CxContentBrowserAssetTrackingParameters extends CortexElement {
  ...
  private emitValuesChange(next: Parameter[]) {
    this.emit('cx-content-browser-asset-tracking-parameters-change', {
      detail: { values: next },
    });
  }
  ...
  render(): TemplateResult {
    const rows = repeat(
      ...
      (_param, index) => index,
      (param, index) => html`
        <cx-space class="content-browser-asset-tracking-parameters__row" spacing="small" wrap="nowrap" align-items="center">
          <cx-input-group>
            <cx-input
