...
function getSearchInput(el: CxDamViewControlBar): CxInput {
  return el.shadowRoot!.querySelector<CxInput>('cx-input.control-bar__search-input')!;
}

describe('dam-view-control-bar', () => {
  let el: CxDamViewControlBar;

  beforeEach(async () => {
    el = await fixture(html`<cx-dam-view-control-bar></cx-dam-view-control-bar>`);
  });
  ...
  describe('initial state', () => {
    ...
    it('renders filter, view mode, and sort controls', () => {
      expect(el.shadowRoot!.querySelector('cx-dam-view-control-filter')).to.exist;
      expect(el.shadowRoot!.querySelector('cx-dam-view-control-view')).to.exist;
      expect(el.shadowRoot!.querySelector('cx-dam-view-control-sort')).to.exist;
    });
  });
  ...
  describe('props forwarded to child controls', () => {
    it('passes view to cx-dam-view-control-view', async () => {
      el.view = 'large';
      await elementUpdated(el);
      const viewCtl = el.shadowRoot!.querySelector('cx-dam-view-control-view');
      expect(viewCtl).to.have.attribute('view', 'large');
    });

    it('passes sort-order-name and sort-direction to cx-dam-view-control-sort', async () => {
      el.sortOrderName = 'name';
      el.sortDirection = 'ascending';
      await elementUpdated(el);
      const sortCtl = el.shadowRoot!.querySelector('cx-dam-view-control-sort');
    ...
    it('passes loading to filter and sort', async () => {
      el.loading = true;
      await elementUpdated(el);
      expect(el.shadowRoot!.querySelector('cx-dam-view-control-filter')).to.have.attribute('loading');
      expect(el.shadowRoot!.querySelector('cx-dam-view-control-sort')).to.have.attribute('loading');
    });

    it('passes is-see-through to view control', async () => {
      el.isSeeThrough = true;
      await elementUpdated(el);
      expect(el.shadowRoot!.querySelector('cx-dam-view-control-view')).to.have.attribute(
        'is-see-through',
      );
    });
  });

  describe('cx-dam-view-control-bar-search-change', () => {
    it('emits with searchText when cx-input fires cx-change', async () => {
      const input = getSearchInput(el);
      input.value = 'asset query';
      await elementUpdated(input);

      const evtPromise = oneEvent(el, 'cx-dam-view-control-bar-search-change');
      input.dispatchEvent(
