...
export default class CxContentBrowserBrowser extends CortexElement {
  ...
  private handleCollectionsSummaryClick(): void {
    this.collectionsOpen = !this.collectionsOpen;
  }

  private handleCollectionsSummaryKeyDown(event: KeyboardEvent): void {
      ...
      >
        ...
        <cx-space direction="vertical" spacing="small" wrap="nowrap">
          ...
          ${when(
            ...
            () => html`
              <div class=${classMap({
                browser__collections: true,
                'browser__collections--open': this.collectionsOpen,
              })}>
                <div
                  class="browser__collections-summary"
                  id="collections-summary"
                  role="button"
                  tabindex="0"
                  aria-expanded=${this.collectionsOpen ? 'true' : 'false'}
                  aria-controls="collections-content"
                  @click=${this.handleCollectionsSummaryClick}
                  @keydown=${this.handleCollectionsSummaryKeyDown}
                >
                  <cx-typography variant="body3">${this.localize.term('collections')}</cx-typography>
                  <cx-icon class="browser__collections-summary-icon" name="chevron_right"></cx-icon>
                </div>
                <div class="browser__collections-content" id="collections-content" role="region" aria-labelledby="collections-summary">
