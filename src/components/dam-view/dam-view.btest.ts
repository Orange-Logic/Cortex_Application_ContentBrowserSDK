...
async function fixtureWithMock(
  ...
  const el = await fixture<CxDamView>(template);
...
function getFormatDialog(el: CxDamView) {
  return el.shadowRoot!.querySelector('cx-dam-view-format-dialog') as CxDamViewFormatDialog;
}

function getGrid(el: CxDamView) {
  return el.shadowRoot!.querySelector('cx-dam-view-grid');
}

function getControlBar(el: CxDamView) {
  return el.shadowRoot!.querySelector('cx-dam-view-control-bar') as CxDamViewControlBar;
}

function getDamViewResizeObserver(el: CxDamView) {
  return el.shadowRoot!.querySelector('cx-resize-observer')!;
}

function getDamViewContentHost(el: CxDamView) {
  return el.shadowRoot!.querySelector('.dam-view__content')!;
}
...
function dispatchContentResize(el: CxDamView, width: number, target: Element) {
  const entry = {
    borderBoxSize: [],
    contentBoxSize: [],
    contentRect: {
  ...
  getDamViewResizeObserver(el).dispatchEvent(
...
function getBrowser(el: CxDamView) {
  return el.shadowRoot!.querySelector('cx-dam-view-browser');
}
...
describe('dam-view', () => {
  ...
  it('renders header, control bar, grid, and format dialog when the fetch controller is ready', async () => {
    const { el } = await fixtureWithMock(html`<cx-dam-view base-url="https://api.test"></cx-dam-view>`);

    expect(getDamViewResizeObserver(el)).to.exist;
    expect(el.shadowRoot!.querySelector('cx-dam-view-header')).to.exist;
    expect(el.shadowRoot!.querySelector('cx-dam-view-control-bar')).to.exist;
  ...
  it('sets isMobile from content width, passes is-mobile to control bar, and switches format dialog to drawer', async () => {
    ...
    const content = getDamViewContentHost(el);
    expect(el.isMobile).to.be.false;
    expect(getControlBar(el).isMobile).to.be.false;
    expect(getFormatDialog(el).variant).to.equal(DamViewFormatDialogVariant.Dialog);
    ...
    expect(getFormatDialog(el).variant).to.equal(DamViewFormatDialogVariant.Drawer);
  ...
  it('ignores cx-resize when the entry target is not dam-view content', async () => {
    const { el } = await fixtureWithMock(html`<cx-dam-view></cx-dam-view>`, {
      parameters: minimalFetchParameters(),
    });
  ...
  it('renders unauthorized message with warning icon when not logged in', async () => {
    ...
    const message = el.shadowRoot!.querySelector('.dam-view__message');
    expect(message).to.exist;
    expect(message!.querySelector('cx-icon.dam-view__message__icon')).to.have.attribute('name', 'warning');
    const typography = message!.querySelector('cx-typography.dam-view__message__text');
    expect(typography).to.exist;
    expect(typography!.textContent?.replaceAll(/\s+/g, ' ').trim()).to.equal('Unauthorized');

    expect(el.shadowRoot!.querySelector('cx-dam-view-header')).to.be.null;
    expect(el.shadowRoot!.querySelector('cx-dam-view-control-bar')).to.be.null;
  ...
  it('does not render the folder browser when parameters are absent', async () => {
    ...
    expect(el.shadowRoot!.querySelector('cx-dam-view-browser')).to.be.null;
  });
  ...
  it('renders the folder browser when parameters include collectionPath', async () => {
    ...
    expect(el.shadowRoot!.querySelector('cx-dam-view-browser')).to.exist;
  });

  it('passes can-favorite and can-use-proxies to the format dialog', async () => {
    const { el } = await fixtureWithMock(html`
      <cx-dam-view .canFavorite=${false} .canUseProxies=${false}></cx-dam-view>
    `);
  ...
  it('opens the format dialog after a grid click when fetchAssetByID resolves', async () => {
    ...
    const { el } = await fixtureWithMock(html`<cx-dam-view .canFavorite=${false}></cx-dam-view>`, {
    ...
    getGrid(el)!.dispatchEvent(
      new CustomEvent('cx-dam-view-grid-click', {
        bubbles: true,
        composed: true,
        detail: { id: asset.id },
      }),
  ...
  it('does not open the format dialog when fetchAssetByID resolves undefined', async () => {
    const asset = makeAsset();
    const { el, mock } = await fixtureWithMock(html`<cx-dam-view></cx-dam-view>`, {
  ...
  it('clears selectedAssetId when the format dialog fires cx-dam-view-format-dialog-close', async () => {
    ...
    getFormatDialog(el).dispatchEvent(
      new CustomEvent('cx-dam-view-format-dialog-close', {
        bubbles: true,
        composed: true,
      }),
    );
  ...
  it('emits cx-dam-view-selected-asset after proxy confirm', async () => {
    const asset = makeAsset();
    const { el, mock } = await fixtureWithMock(html`<cx-dam-view></cx-dam-view>`, {
      items: [asset],
      totalCount: 1,
    ...
    const dialog = getFormatDialog(el);
    const p = oneEvent(el, 'cx-dam-view-selected-asset');

    dialog.dispatchEvent(
      new CustomEvent('cx-dam-view-format-dialog-proxy-confirm', {
        bubbles: true,
        composed: true,
        detail: {
          asset,
  ...
  it('handleFormatConfirm calls getAssetLink, hides the dialog, and emits selected asset with transformation metadata', async () => {
    ...
    dialog.dispatchEvent(
      new CustomEvent('cx-dam-view-format-dialog-format-confirm', {
        bubbles: true,
        composed: true,
        detail: {
          asset,
  ...
  it('merges ScrubUrl into selected payload when extra-fields requests it', async () => {
    ...
    const { el } = await fixtureWithMock(
      html`<cx-dam-view extra-fields="ScrubUrl"></cx-dam-view>`,
      {
  ...
  it('calls setVersionHistory on the format dialog when version history is opened', async () => {
    ...
    dialog.dispatchEvent(
      new CustomEvent('cx-dam-view-format-dialog-version-history-open', {
        bubbles: true,
        composed: true,
  ...
  it('calls setIsFavorite on the format dialog when favorite add succeeds', async () => {
    ...
    dialog.dispatchEvent(
      new CustomEvent('cx-dam-view-format-dialog-favorite-change', {
        bubbles: true,
        composed: true,
  ...
  it('updates grid view when the control bar emits a view change', async () => {
    ...
    bar!.dispatchEvent(
      new CustomEvent('cx-dam-view-control-view-change', {
        bubbles: true,
        composed: true,
  ...
  describe('runFirstUpdated: defaultPageSize from default-grid-view (no view-change event)', () => {
    it('uses 15 for Large', async () => {
      const { el, mock } = await fixtureWithMock(html`
        <cx-dam-view default-grid-view=${GridView.Large}></cx-dam-view>
      `);
      ...
      getGrid(el)!.dispatchEvent(
        new CustomEvent('cx-dam-view-grid-scroll-end', {
          bubbles: true,
          composed: true,
        }),
      );
    ...
    it('uses 20 for Medium', async () => {
      const { el, mock } = await fixtureWithMock(html`
        <cx-dam-view default-grid-view=${GridView.Medium}></cx-dam-view>
      `);
    ...
    it('uses 30 for Small', async () => {
      const { el, mock } = await fixtureWithMock(html`
        <cx-dam-view default-grid-view=${GridView.Small}></cx-dam-view>
      `);
  ...
  it('handleSortOrderChange updates lastRequest, newlyChangedOption, and refetches', async () => {
    ...
    bar!.dispatchEvent(
      new CustomEvent('cx-dam-view-control-sort-order-change', {
        bubbles: true,
        composed: true,
        detail: {
  ...
  it('handleFilterChange updates selectedFacets, resets start, and refetches', async () => {
    ...
    /** Empty map avoids dam-view-control-filter repeating tags before mappedDisplayNames exists. */
    const selection: Record<string, string[]> = {};

    bar!.dispatchEvent(
      new CustomEvent('cx-dam-view-control-filter-change', {
        bubbles: true,
        composed: true,
        detail: { selection },
      }),
  ...
  it('handleFolderSelectionChange updates folderId, folderTitle, start, and refetches', async () => {
    ...
    expect(el.shadowRoot!.querySelector('cx-dam-view-header')).to.have.attribute(
      'folder-title',
      'New folder',
    );
  });
  ...
  it('handleSearchChange updates searchText, resets start, and refetches', async () => {
    ...
    bar!.dispatchEvent(
      new CustomEvent('cx-dam-view-control-bar-search-change', {
        bubbles: true,
        composed: true,
