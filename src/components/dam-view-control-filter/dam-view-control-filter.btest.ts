import './dam-view-control-filter';

import { elementUpdated, expect, fixture, html, oneEvent } from '@open-wc/testing';

import { type Facet, OptionType } from '@/types/dam-view';

import type CxDamViewControlFilter from './dam-view-control-filter';
...
describe('dam-view-control-filter', () => {
  let el: CxDamViewControlFilter;

  beforeEach(async () => {
    el = await fixture(html`<cx-dam-view-control-filter></cx-dam-view-control-filter>`);
  });
  ...
  describe('initial state', () => {
    ...
    it('does not render facet list when availableFacets is empty', () => {
      expect(el.shadowRoot!.querySelector('cx-dam-view-control-filter-facet')).to.be.null;
    });
  });
  ...
  describe('with facets and selections', () => {
    ...
    it('renders facet components for available facets that exist in facets', () => {
      const facetEl = el.shadowRoot!.querySelector('cx-dam-view-control-filter-facet');
    ...
    it('emits cx-dam-view-control-filter-change with empty selection when clear all is clicked', async () => {
      const clearBtn = el.shadowRoot!.querySelector('cx-button[variant="text"]');
      expect(clearBtn).to.exist;
      const evtPromise = oneEvent(el, 'cx-dam-view-control-filter-change');
  ...
  describe('available facets vs facets payload', () => {
    it('does not render a facet control when availableFacets has no matching entry in facets', async () => {
      ...
      const facetEls = el.shadowRoot!.querySelectorAll('cx-dam-view-control-filter-facet');
  ...
  describe('event guards', () => {
    ...
    it('ignores cx-remove when tag has no data-value', async () => {
      let fired = false;
      el.addEventListener('cx-dam-view-control-filter-change', () => {
        fired = true;
      });
