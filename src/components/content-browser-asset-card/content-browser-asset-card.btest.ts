...
import type ContentBrowserAssetCard from './content-browser-asset-card';

const PLACEHOLDER_TEXT = 'empty';
const THUMB = 'https://placehold.co/120x80';

function getCard(el: ContentBrowserAssetCard) {
  return el.shadowRoot!.querySelector('cx-card')!;
}

function getPreview(el: ContentBrowserAssetCard) {
  return el.shadowRoot!.querySelector('cx-content-browser-asset-preview')!;
}

describe('content-browser-asset-card', () => {
  ...
  describe('initial state', () => {
    ...
    it('applies view and state classes on the card', () => {
      ...
      expect(card.classList.contains('content-browser-asset-card')).to.be.true;
      expect(card.classList.contains('content-browser-asset-card--medium')).to.be.true;
    ...
    it('applies selected class on cx-card when selected is true', async () => {
      ...
      await elementUpdated(el);
      const card = getCard(el);
      expect(card.classList.contains('content-browser-asset-card--selected')).to.be.true;
    });
  ...
  describe('content-browser-asset-preview integration', () => {
    beforeEach(async () => {
      el.imageUrl = THUMB;
    ...
    it('marks the card disabled when in cold storage', async () => {
      ...
      await elementUpdated(el);
      expect(getCard(el).classList.contains('content-browser-asset-card--disabled')).to.be.true;
  ...
  describe('title row', () => {
    it('shows asset name when showTitle and assetName are set', async () => {
      ...
      const nameTypo = el.shadowRoot!.querySelector(
        '.content-browser-asset-card__name cx-typography',
      );
    ...
    it('shows placeholder when showTitle is off', async () => {
      el.assetName = 'Hidden';
      await elementUpdated(el);
      const placeholder = el.shadowRoot!.querySelector('.content-browser-asset-card__placeholder');
  ...
  describe('selection', () => {
    it('adds selected class and shows checkbox when selected', async () => {
      el.selected = true;
      await elementUpdated(el);
      expect(getCard(el).classList.contains('content-browser-asset-card--selected')).to.be.true;
      const overlay = el.shadowRoot!.querySelector('.content-browser-asset-card__checkbox');
  ...
  describe('tags', () => {
    it('renders up to five tags when showTags is true', async () => {
      ...
      el.tags = ['a', 'b', 'c', 'd', 'e', 'f'];
      await elementUpdated(el);
      const tags = el.shadowRoot!.querySelectorAll('.content-browser-asset-card__tags cx-tag');
    ...
    it('renders empty tag container when showTags and tags are empty', async () => {
      ...
      el.tags = [];
      await elementUpdated(el);
      const clamp = el.shadowRoot!.querySelector('.content-browser-asset-card__tags');
  ...
  describe('size and dimensions row', () => {
    ...
    it('shows placeholder when dimensions row is on but has nothing to show', async () => {
      ...
      el.showSize = false;
      await elementUpdated(el);
      const rows = el.shadowRoot!.querySelectorAll('.content-browser-asset-card__info');
