...
import type CxDamViewAssetPreview from './dam-view-asset-preview';
import { getMediaIcon } from './dam-view-asset-preview';
import CxDamViewAssetPreviewVideo from './dam-view-asset-preview-video/dam-view-asset-preview-video';

describe('dam-view-asset-preview', () => {
  let el: CxDamViewAssetPreview;

  beforeEach(async () => {
    el = await fixture(html`<dam-view-asset-preview></dam-view-asset-preview>`);
  });
  ...
  describe('when no imageUrl is set', () => {
    for (const docType of Object.values(MediaType)) {
      it(`renders the thumbnail for ${docType}`, async () => {
        ...
        const thumbnail = el.shadowRoot!.querySelector('[data-testid="dam-view-asset-preview__thumbnail--other"]');
        expect(thumbnail).to.exist;
        expect(thumbnail).to.have.class('dam-view-asset-preview__thumbnail');
        expect(thumbnail).to.have.class('dam-view-asset-preview__thumbnail--other');
  ...
  describe('when imageUrl is set', () => {
    describe('when in cold storage', () => {
      it('renders the cold storage thumbnail', async () => {
        ...
        await elementUpdated(el);
        const thumbnail = el.shadowRoot!.querySelector('[data-testid="dam-view-asset-preview__thumbnail--cold-storage"]');
        expect(thumbnail).to.have.class('dam-view-asset-preview__thumbnail');
        expect(thumbnail).to.have.class('dam-view-asset-preview__thumbnail--other');
        const text = el.shadowRoot!.querySelector('[data-testid="dam-view-asset-preview__thumbnail--cold-storage-text"]');
        expect(text?.textContent.trim()).to.equal(el['localize'].term('assetInColdStorage'));
        const textSmall = el.shadowRoot!.querySelector('[data-testid="dam-view-asset-preview__thumbnail--cold-storage-text-small"]');
    ...
    describe('when doc type is video', () => {
      let video: CxDamViewAssetPreviewVideo | null;
      beforeEach(async () => {
        el.docType = MediaType.Video;
        el.imageUrl = 'https://placehold.co/600x400';
        await elementUpdated(el);
        video = el.shadowRoot!.querySelector('cx-dam-view-asset-preview-video');
      });
    ...
    describe('when doc type is image', () => {
      it('renders the image', async () => {
        ...
        el.imageUrl = 'https://placehold.co/600x400';
        await elementUpdated(el);
        const container = el.shadowRoot!.querySelector('[data-testid="dam-view-asset-preview__representative-container"]');
        expect(container).to.exist;
        expect(container).to.have.class('dam-view-asset-preview__representative-container');
        const image = container!.querySelector('img');
        expect(image).to.exist;
        expect(image).to.have.class('dam-view-asset-preview__representative');
      ...
      it('renders the animated image when extension is gif and originalUrl is set', async () => {
        ...
        expect(container).to.exist;
        expect(container).to.have.class('dam-view-asset-preview__representative-container');
        expect(container).to.have.class('dam-view-asset-preview__representative-container--animated');
        const animatedImage = container!.querySelector('.dam-view-asset-preview__representative--animated');
