...
import styles from './dam-view-header.styles';
 ...
 *
 * @event cx-dam-view-header-close - Emitted when the close button is clicked (when shown in popup).
 * @event cx-dam-view-header-logout - Emitted when the user selects logout.
 */
@customElement('cx-dam-view-header')
export default class CxDamViewHeader extends CortexElement {
  ...
  private handleCloseClick(): void {
    this.emit('cx-dam-view-header-close');
  }
  ...
  private handleMenuSelect(event: CxSelectEvent<CxMenuItem>): void {
    ...
    if (item?.value === LOGOUT_ITEM_VALUE) {
      this.emit('cx-dam-view-header-logout');
    }
  }
