import './dam-view-header';

import { elementUpdated, expect, fixture, html, oneEvent } from '@open-wc/testing';

import type CxDamViewHeader from './dam-view-header';

const AVATAR_URL = 'https://placehold.co/40x40';
const USER_NAME = 'Test User';

describe('dam-view-header', () => {
  let el: CxDamViewHeader;

  beforeEach(async () => {
    el = await fixture(html`<cx-dam-view-header></cx-dam-view-header>`);
  });

  describe('initial state', () => {
    it('is accessible when folder title is set', async () => {
      const header = await fixture(
        html`<cx-dam-view-header folder-title="Assets"></cx-dam-view-header>`,
      );
  ...
  describe('user area with logout', () => {
    ...
    it('emits cx-dam-view-header-logout when logout is selected', async () => {
      ...
      const evtPromise = oneEvent(el, 'cx-dam-view-header-logout');
      menu.dispatchEvent(
    ...
    it('does not emit logout for other menu selections', async () => {
      ...
      let fired = false;
      el.addEventListener('cx-dam-view-header-logout', () => {
        fired = true;
      });
      menu.dispatchEvent(
        new CustomEvent('cx-select', {
  ...
  describe('close button', () => {
    ...
    it('emits cx-dam-view-header-close when close is clicked', async () => {
      const btn = el.shadowRoot!.querySelector('cx-icon-button[name="close"]');
      expect(btn).to.exist;
      ...
      const evtPromise = oneEvent(el, 'cx-dam-view-header-close');
