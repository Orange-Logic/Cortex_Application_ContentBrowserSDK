...
import { ASSET_SIZE, GridView } from '@/types/dam-view';
...
import CxDamViewAssetCard from '../dam-view-asset-card/dam-view-asset-card';
import styles from './dam-view-grid.styles';

import type { CxResizeEvent } from '@/events';
import type { CSSResultGroup } from 'lit';

@customElement('dam-view-grid')
export default class CxDamViewGrid extends CortexElement {
  static readonly styles: CSSResultGroup = [componentStyles, styles];

  static readonly dependencies = {
    'cx-dam-view-asset-card': CxDamViewAssetCard,
  ...
  private calculatePageSize(width: number, height: number, force: boolean = false) {
    ...
    this.emit('cx-dam-view-grid-resize', {
      detail: {
        columnCount,
        rowCount,
      },
  ...
  private handleClick(event: MouseEvent) {
    const target = event.target as CxDamViewAssetCard;
    const assetId = target.assetId;

    if (assetId) {
      this.emit('cx-dam-view-grid-click', {
        detail: {
          id: assetId,
        },
      });
  ...
  private handleScrollEnd() {
    if (this.hasMore) {
      this.emit('cx-dam-view-grid-scroll-end');
    }
  }
  ...
  render() {
    return html`
      ...
      <cx-resize-observer @cx-resize=${this.handleResize}>
        <div class="dam-view-grid">
          ${when(this.empty,
            () => html`
              <div class="dam-view-grid__empty">
